...
4. **Code review** — correctness bugs and security problems in the diff (authz missing on a route, identity taken from client input, secrets in code/logs, unvalidated input reaching a query, partial writes, swallowed errors). Label each finding `[BLOCKING]` / `[SUGGESTION]` / `[NIT]` / `[QUESTION]` per `docs/code-review.md`; describe the defect and a failing scenario; do not prescribe an architecture.

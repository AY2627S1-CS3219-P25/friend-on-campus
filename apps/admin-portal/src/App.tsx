...
import { SupplierDTO, SupplierCategory } from '@campus-errand/common-dtos';
    ...
    <div className="flex h-screen bg-slate-100 text-slate-800">
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0">
        <div className="p-5 border-b border-slate-800 flex items-center space-x-3">
          <div className="w-9 h-9 rounded-lg bg-nus-orange flex items-center justify-center font-black text-white text-xl">
            A
          </div>
          <div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 px-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">Predefined pickup points and store directory for requesters</p>
          </div>

          {activeNav === 'suppliers' && (
            <button
              onClick={() => setIsAddOpen(true)}
              className="bg-nus-orange hover:bg-orange-600 text-white font-bold text-xs px-4 py-2 rounded-lg shadow flex items-center space-x-1.5 transition"
            >
          ...
          {activeNav === 'suppliers' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <span className="text-xs text-slate-500 font-semibold">Campus Zones</span>
                  <p className="text-2xl font-black text-nus-blue mt-1">4 Zones</p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <span className="text-xs text-slate-500 font-semibold">Categories</span>
                  <p className="text-2xl font-black text-amber-600 mt-1">4 Types</p>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between space-x-4">
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 text-xs rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-nus-blue"
                  />
                </div>

                <div className="flex space-x-2">
                  {['ALL', 'Beverages', 'Food', 'Printing', 'Parcels'].map((cat) => (
                    <button
                      key={cat}
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                      ...
                      <th className="p-3.5">Exact Location</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((s) => (
                      <tr key={s.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="p-3.5">
                          <span className="bg-blue-50 text-nus-blue px-2 py-0.5 rounded font-bold">
                            {s.campusZone}
                          </span>
                        </td>
                        <td className="p-3.5 text-slate-600">{s.exactLocation}</td>
                        <td className="p-3.5">
                        ...
                        <td className="p-3.5 text-right">
                          <button
                            ...
                            className="text-xs font-semibold text-slate-600 hover:text-nus-blue underline"
                          >
      ...
      {isAddOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          ...
          >
            <h3 className="font-bold text-base text-slate-900">Add New Campus Location</h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  ...
                  <option value="PGPR">PGPR</option>
                  <option value="FASS">FASS</option>
                  <option value="Central Lib">Central Lib</option>
                </select>
              </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Level 1 Town Plaza next to FairPrice"
            ...
            <div className="flex justify-end space-x-2 pt-2 border-t border-slate-100">
              ...
              <button
                type="submit"
                className="bg-nus-orange hover:bg-orange-600 text-white font-bold text-xs px-4 py-2 rounded-lg shadow"
              >
                Save Location
              </button>
            </div>
